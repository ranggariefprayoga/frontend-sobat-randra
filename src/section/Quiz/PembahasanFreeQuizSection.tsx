...
import { useGetFreeQuizSessionQuestionDetailForUser, useGetUserAnswerCorrectionForPembahasan } from "@/lib/api/quizHistory.api";
...
export default function PembahasanFreeQuizSection() {
  ...
  const { data: detailQuizSession, isLoading: isLoadingSession, error } = useGetFreeQuizSessionQuestionDetailForUser(sessionId, productTryOutId, questionId);
  ...
  const handlePreviousSoal = (previousQuestionId: number | null | undefined) => {
    if (previousQuestionId) {
      router.push(`/pembahasan-quiz-free?sess=${sessionId}&ptid=${productTryOutId}&qid=${previousQuestionId}`);
    }
  };
  const handleNextSoal = (nextQuestionId: number | null | undefined) => {
    if (nextQuestionId === null || nextQuestionId === undefined) {
      router.push(`/pembahasan-quiz-free?sess=${sessionId}&ptid=${productTryOutId}&qid=${0}`);
    }

    if (nextQuestionId) {
      router.push(`/pembahasan-quiz-free?sess=${sessionId}&ptid=${productTryOutId}&qid=${nextQuestionId}`);
    }
  };
