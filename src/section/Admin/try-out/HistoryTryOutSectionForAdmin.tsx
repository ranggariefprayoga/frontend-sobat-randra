...
export default function HistoryTryOutSectionForAdmin() {
  ...
  return (
    <LayoutBackgroundWhite>
      ...
      <TitleComponent title="History Pengerjaan Try Out" />
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4 md:px-24 mt-8">
          {filteredProducts.map((product: TryOutProductModel) => (
            <CardTryOutForAdmin key={product.id} product={product} customLink={`/admin/tryout/history-tryout/${product.id}`} buttonText="Lihat yang Sudah Mengerjakan" />
          ))}
        </div>
      ) : (
