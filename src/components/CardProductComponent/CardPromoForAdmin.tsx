...
import { useDeleteProductPromoById } from "@/lib/api/productPromo.api";
...
export default function CardPromoForAdmin({ product, customLink, buttonText = "Lihat Detail" }: ProductCardProps) {
  const [open, setOpen] = useState(false);
  const router = useRouter();
  const deleteMutation = useDeleteProductPromoById();
      ...
      <div className="absolute top-2 right-2 z-20">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent aria-describedby={undefined}>
            ...
            <p className="text-sm text-gray-600">Produk Promo ini akan dihapus secara permanen.</p>
