...
import { useDeleteVideoBelajarProductById } from "@/lib/api/productVideoBelajar.api";
...
export default function CardVideoBelajarForAdmin({ product, customLink, buttonText = "Lihat Detail" }: ProductCardProps) {
  const [open, setOpen] = useState(false);
  const router = useRouter();
  const deleteMutation = useDeleteVideoBelajarProductById();
  ...
  const handleDelete = () => {
    deleteMutation.mutate(product.id, {
      onSuccess: () => {
        toast.success("Video Belajar berhasil dihapus.");
        setOpen(false);
      },
      onError: () => {
        toast.error("Gagal menghapus Video Belajar.");
      },
    });
  };
      ...
      <div className="absolute top-2 right-2 z-20">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent aria-describedby={undefined}>
            ...
            <p className="text-sm text-gray-600">Produk Video Belajar ini akan dihapus secara permanen.</p>
