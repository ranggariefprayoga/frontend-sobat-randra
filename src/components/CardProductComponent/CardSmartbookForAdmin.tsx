...
import { useDeleteSmartbookProductById } from "@/lib/api/productSmartbook.api";
...
export default function CardSmartbookForAdmin({ product, customLink, buttonText = "Lihat Detail" }: ProductCardProps) {
  const [open, setOpen] = useState(false);
  const router = useRouter();
  const deleteMutation = useDeleteSmartbookProductById();
  ...
  const handleDelete = () => {
    deleteMutation.mutate(product.id, {
      onSuccess: () => {
        toast.success("CheatSheet berhasil dihapus.");
        setOpen(false);
      },
      onError: () => {
        toast.error("Gagal menghapus CheatSheet.");
      },
    });
  };
      ...
      <div className="absolute top-2 right-2 z-20">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent aria-describedby={undefined}>
            ...
            <p className="text-sm text-gray-600">Produk Smartbook ini akan dihapus secara permanen.</p>
