...
import { useDeleteBimbelBarengProductById } from "@/lib/api/productBimbelBareng.api";
...
export default function CardBimbelBarengForAdmin({ product, customLink, buttonText = "Lihat Detail" }: ProductCardProps) {
  const [open, setOpen] = useState(false);
  const router = useRouter();
  const deleteMutation = useDeleteBimbelBarengProductById();
  ...
  const handleDelete = () => {
    deleteMutation.mutate(product.id, {
      onSuccess: () => {
        toast.success("Bimbel Bareng berhasil dihapus.");
        setOpen(false);
      },
      onError: () => {
        toast.error("Gagal menghapus Bimbel Bareng.");
      },
    });
  };
      ...
      <div className="absolute top-2 right-2 z-20">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent aria-describedby={undefined}>
            ...
            <p className="text-sm text-gray-600">Produk Bimbel Bareng ini akan dihapus secara permanen.</p>
