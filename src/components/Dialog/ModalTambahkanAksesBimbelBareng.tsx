...
import { useCreateBimbelBarengAccess } from "@/lib/api/productBimbelBarengAccess.api";

interface CreateBimbelBarengAccessModalProps {
  product_bimbel_bareng_id: number;
  onSuccess?: () => void;
}

export default function ModalTambahkanAksesBimbelBareng({ product_bimbel_bareng_id, onSuccess }: CreateBimbelBarengAccessModalProps) {
  const [open, setOpen] = useState(false);
  const [userEmail, setUserEmail] = useState("");
  const [phoneNumber, setPhoneNumber] = useState("");

  const createMutation = useCreateBimbelBarengAccess();
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createMutation.mutateAsync({
        product_bimbel_bareng_id: product_bimbel_bareng_id,
        user_email: userEmail.trim(),
        phone_number: phoneNumber.trim(),
      });
      toast.success("Akses bimbel bareng berhasil ditambahkan");
      setOpen(false);
      setUserEmail("");
      setPhoneNumber("");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Tambah Akses bimbel bareng</DialogTitle>
