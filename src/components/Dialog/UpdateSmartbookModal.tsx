...
import { useUpdateSmartbookProduct } from "@/lib/api/productSmartbook.api";

export function UpdateProductSmartbookModal({ initialData }: { initialData?: ProductSmartbookResponse }) {
  ...
  const updateMutation = useUpdateSmartbookProduct();
  ...
  const handleSubmit = async () => {
    ...
    if (initialData?.id) {
      updateMutation.mutate(
        { product_smartbook_id: initialData.id, data: payloadData, banner_image: bannerImage },
        {
          onSuccess: () => {
            setOpen(false);
            toast.success("Smartbook berhasil diperbarui.");
          },
          onError: () => {
            toast.error("Gagal memperbarui Smartbook.");
          },
        }
      );
    }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button>Edit Smartbook Premium</Button>
      </DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Edit Smartbook Premium</DialogTitle>
          <DialogDescription>Perbarui informasi Smartbook di bawah ini.</DialogDescription>
...
export default UpdateProductSmartbookModal;
