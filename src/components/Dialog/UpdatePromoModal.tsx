...
import { useUpdateProductPromo } from "@/lib/api/productPromo.api";

export function UpdateProductPromoModal({ initialData }: { initialData?: ProductPromoResponse }) {
  ...
  const updateMutation = useUpdateProductPromo();
  ...
  const handleSubmit = async () => {
    ...
    if (initialData?.id) {
      updateMutation.mutate(
        { product_promo_id: initialData.id, data: payloadData, banner_image: bannerImage },
        {
          onSuccess: () => {
            setOpen(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button>Edit Promo Premium</Button>
      </DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Edit Promo Premium</DialogTitle>
          <DialogDescription>Perbarui informasi Promo di bawah ini.</DialogDescription>
        ...
        <div className="space-y-4">
          ...
          <div className="mb-4 space-y-1.5">
            <Label>
              Deskripsi Promo<span className="text-red-600">*</span>
            </Label>
...
export default UpdateProductPromoModal;
