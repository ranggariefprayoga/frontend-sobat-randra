...
import { getAllQuizSessionByUser, getHistorySessionDetailByUser, getHistorySessionDetailByUserForPembahasan, WebResponseDetailHistorySession } from "@/model/quiz-history.model";
...
export const useGetUserAnswerCorrectionForPembahasan = (try_out_session_id: number, product_try_out_id: number) => {
  return useQuery<WebResponseDetailHistorySession<getHistorySessionDetailByUserForPembahasan[]>, Error>({
    queryKey: ["quiz-session-question-detail-pembahasan", try_out_session_id, product_try_out_id],
    queryFn: async () => {
      const res = await axios.get(`${API_BASE_URL}/api/quiz-history/session/${try_out_session_id}/product/${product_try_out_id}/user-answers`, { withCredentials: true });
      return res.data;
    },
    retry: false,
  });
