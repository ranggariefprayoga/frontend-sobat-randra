...
export const useUpdateVideoBelajarProduct = (): UseMutationResult<WebResponse<ProductVideoBelajarResponse>, Error, { product_video_belajar_id: number; data: updateProductVideoBelajarRequest; banner_image: File | null }> => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ product_video_belajar_id, data, banner_image }) => {
      const formData = new FormData();
